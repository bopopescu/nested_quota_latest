...
def quota_set(id, include_server_group_quotas=True):
    res = {'quota_set': {'id': id, 'metadata_items': 0,
           'ram': 0, 'floating_ips': 0, 'fixed_ips': 0,
           'instances': 0, 'injected_files': 0, 'cores': 0,
           'injected_file_content_bytes': 0,
           'security_groups': 0, 'security_group_rules': 0,
           'key_pairs': 0, 'injected_file_path_bytes': 0}}
    if include_server_group_quotas:
        res['quota_set']['server_groups'] = 0
        res['quota_set']['server_group_members'] = 0
    return res
...
class QuotaSetsTestV21(BaseQuotaSetsTest):
    ...
    def setUp(self):
        ...
        if self.include_server_group_quotas:
            self.default_quotas['server_groups'] = 0
            self.default_quotas['server_group_members'] = 0
    ...
    def test_format_quota_set(self):
        ...
        self.assertEqual(qs['instances'], 0)
        self.assertEqual(qs['cores'], 0)
        self.assertEqual(qs['ram'], 0)
        self.assertEqual(qs['floating_ips'], 0)
        self.assertEqual(qs['fixed_ips'], 0)
        self.assertEqual(qs['metadata_items'], 0)
        self.assertEqual(qs['injected_files'], 0)
        self.assertEqual(qs['injected_file_path_bytes'], 0)
        self.assertEqual(qs['injected_file_content_bytes'], 0)
        self.assertEqual(qs['security_groups'], 0)
        self.assertEqual(qs['security_group_rules'], 0)
        self.assertEqual(qs['key_pairs'], 0)
        if self.include_server_group_quotas:
            self.assertEqual(qs['server_groups'], 0)
            self.assertEqual(qs['server_group_members'], 0)
    ...
    def test_quotas_update_as_admin(self):
        ...
        req = fakes.HTTPRequestV3.blank('/v3/fake4/os-quota-sets/update_me',
                                      use_admin_context=True)
        ctxt = req.environ['nova.context']
        ctxt.auth_token = 'fake_token'
	ctxt.project_id = 'update_me'
...
class UserQuotasTestV21(BaseQuotaSetsTest):
    ...
    def test_user_quotas_update_as_admin(self):
        ...
        body = {'quota_set': {'instances': 0, 'cores': 0,
                              ...
                              'key_pairs': 0}}
        if self.include_server_group_quotas:
            body['quota_set']['server_groups'] = 0
            body['quota_set']['server_group_members'] = 0
